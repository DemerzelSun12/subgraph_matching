...
#include "genMatchOrderSingleV.h"
...
std::vector<PhaseSingleV> matchOrder;
...
uint genInit(uint *vLabel_dev, uint *writeRowNum_dev, uint *edgeLabelPartition_dev, uint *neighborsData_dev, 
    ...
    PhaseSingleV &firstPhase = matchOrder[0];
...
uint* genExtEmb(uint *vLabel_dev, uint *writeRowNum_dev, uint *writeRowNum, uint *edgeLabelPartition_dev, uint *neighborsData_dev,
    ...
    PhaseSingleV &curPhase, bool isLastPhase, bool isContinue){
    ...
    if(pairs.size()==2){
        ...
        while(tmppartialRowNum>=maxRowNum){
            ...
            //std::cout<<"begin over size extEmb_1V_NoHash"<<std::endl;
            extEmb_1V_NoHash(vLabel_dev,writeRowNum_dev,edgeLabelPartition_dev,neighborsData_dev,recordPos_dev,auxArray_dev,
            ...
            //std::cout<<"after over size extEmb_1V_NoHash, write row num:"<<tmppartialRowNum<<std::endl;
        }
...
uint* reduction(uint *edgelabelPartition,uint *edgeLabelPartition_dev, uint *vLabel_dev, uint*writeRowNum_dev, uint *writeRowNum,
               uint*baseAddr_dev,uint &stride,uint totSrcNum,uint intervalNum,PhaseSingleV &curPhase,uint *partialEmb_dev,
...
int main(int argc, char *argv[]){
    ...
    //debugNum = std::atoi(argv[3]);
    //debugNumend = std::atoi(argv[4]);
    
    //load graph data base
    ...
    clock_t start, end;
    start = clock();
    genMatchOrderSingleV(queryFileName,matchOrder,repeatFlag,restricts,edgeLabelStat,queryLabel,allEdgeLabelPartitions,baseAddr_dev,stride);
    PhaseSingleV &tmpPhase = matchOrder[0];
    ...
    for(uint i=0;i<matchOrder.size();++i){
        PhaseSingleV &phase = matchOrder[i];
    ...
    for(i=1;i<matchOrder.size();++i){
        ...
        stride = 0;
        PhaseSingleV &curPhase = matchOrder[i];
        ...
        if(curPhase.phaseType=='E'){
            ...
            //cudaDeviceSynchronize();
            //err = cudaGetLastError();
            //std::cout<<"7 "<<i<<" "<<cudaGetErrorString(err)<<std::endl;
            partialRowNum = writeRowNum[0];
            partialEmb_dev = baseAddr_dev+gpuAvailSpace-partialRowNum*embLen;
            //std::cout<<"totWriteRowNum="<<partialRowNum<<" stride="<<stride<<std::endl;
            ...
            //err = cudaGetLastError();
            //std::cout<<"9 "<<i<<" "<<cudaGetErrorString(err)<<std::endl;
            writeRowNum[0] = 0;
            writeRowNum[1] = 0;
            cudaMemcpyAsync(writeRowNum_dev,writeRowNum,sizeof(uint)*2,cudaMemcpyHostToDevice);
            //err = cudaGetLastError();
            ...
            //cudaDeviceSynchronize();
            //err = cudaGetLastError();
            //std::cout<<"5 "<<i<<" "<<cudaGetErrorString(err)<<std::endl;
        }
    }
    cudaDeviceSynchronize();
    end = clock();
